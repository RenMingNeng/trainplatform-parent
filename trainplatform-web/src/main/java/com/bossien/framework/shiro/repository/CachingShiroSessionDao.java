...
public class CachingShiroSessionDao extends CachingSessionDAO {
    ...
    protected void doUpdate(final Session session) {
        ...
        try {
            if (session instanceof ShiroSession) {
                ...
                logger.error("sessionId {} name {} 更新失败", session.getId(), session.getClass().getName());
            }
        } catch (Exception e) {
    ...
    public void unCache(final Serializable sessionId) {
        try {
            ...
            logger.debug("本地 cache中缓存的Session id {} 失效", sessionId);
        } catch (Exception e) {
            logger.error("删除本地 cache中缓存的Session 失败", e);
        }
    }
