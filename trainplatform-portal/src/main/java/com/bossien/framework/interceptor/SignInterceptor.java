package com.bossien.framework.interceptor;
...
public class SignInterceptor implements HandlerInterceptor {
	...
	private static final String key = PropertiesUtils.getValue("key");

	private static final String sign = PropertiesUtils.getValue("sign");

	private static final String TIMESTAMP = PropertiesUtils.getValue("timestamp");
...
//				}
				...
				double millis = (current - longTS); // 时间戳单位 秒
				if (millis < 60000000 && millis > -60000000) { // 60s 内有效
					return true;
				}
			}
		} catch (Exception ex) {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(handler instanceof  HandlerMethod) {
			...
			if(needSign != null){
				...
				String requestSignv2 = sortMap.get(sign);
				String param = sortMap.get("test");
				//TODO 方便测试 上线之前去掉
				if("nosign".equals(param)){
					return true;
				}
